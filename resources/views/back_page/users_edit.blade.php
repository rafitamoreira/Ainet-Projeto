                        ...
                        <form method="POST" action="{{ route('users.update', $user) }}" class="form-group"
                            ...
                            <div class="form-group">
                                ...
                                <select name="tipo" id="tipo">
                                    ...
                                    <option value="E" {{ old('tipo', $user->user_type) == 'E' ? 'selected' : '' }}>
                                        Funcionário
                                    </option>
                                </select>
                                @error('tipo')
                            ...
                            <div class="form-group">
                                ...
                                <input type="file" class="form-control" name="foto_url" id="foto_url"
                                    value="{{ old('foto_url', $user->photo_url) }}">
                                @error('foto_url')
                                    <div class="small text-danger">{{ $message }}</div>
                                @enderror
                            </div>
                            @if (isset($user->foto_url))
                                <img src="{{ asset("storage/fotos/$user->photo_url") }}" width="30%" />
                            @endif
                            <div class="form-group">
